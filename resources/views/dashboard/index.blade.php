...
<div class="container">
	<div class="row">
		<div class="col-md-10 col-md-offset-1">
			<center><h1> All Job Posts</h1></center>
			@if(Auth::check()== true)
			...
			<div class="panel panel-default">
				...
				@elseif((Auth::check()== true) && (Auth::user()->name != $post->author) && ($post->status != "awarded"))
                    ...
                    <div id="notifiererror{{$post->id}}" style="background-color:red;color:white;display:none;">You have Already Apllied for this job</div>
					<div class="toggle">
						...
						<div class="toggle-inner">
							...
							<div class="form-group row col-xs-offset-4">
								<button type="button" class="btn btn-primary title-name" id="submitbid{{$post->id}}" onclick="placebid({{$post->id}},'{{$post->author}}','{{$post->title}}')">Submit Bid</button>
			...
			@endforeach
			<div class="pagination"> {{ $posts->links() }}
...
<script type="text/javascript">
	...
	function placebid(id,author,title){
		$.ajax({
			type: "POST",
			url: "/jobpost",
			data: {postid: id, description: $("#biddesc"+id).val(), amount: $("#amount"+id).val(),  _token: "{{ Session::token() }}", title: title, author: author},
			success: function(result){
				console.log(result);
	   						$("#biddesc"+id).val("");
							$("#amount"+id).val("");
							$(".togglecloser").removeClass("active").closest('.toggle').find('.toggle-inner').slideUp(200);
                            if(result=='Sorry You have already applied for this job'){
                            $("#notifiererror"+id).show();
                            }
                            else {
